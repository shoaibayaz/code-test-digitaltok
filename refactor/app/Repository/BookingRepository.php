...
class BookingRepository extends BaseRepository
{
     ...
     * @return array
     */
    public function getUsersJobsHistory($userId, $page)
    {
        $emergencyJobs = [];
        $noramlJobs = [];
        $numpages = 0;
        ...
        return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => $numpages, 'pagenum' => 0];
    }
    ...
    public function store($user, $data)
    {
        ...
        $certificates = [
            ...
            'normal_certified_in_helth' => 'n_health',
        ];
        
        $selectedCertificates = [];
        foreach ($data['job_for'] as $jobFor) {
            if (in_array($jobFor, $genders)) {
                $data['gender'] = $jobFor;
            }
            if (in_array($jobFor, array_keys($certificates))) {
                $selectedCertificates[] = $jobFor;
            }
        }
        
        if (count($selectedCertificates) > 0) {
            $data['certified'] = $certificates[implode('_', $selectedCertificates)];
        }
        $jobTypes = [
            'rwsconsumer' => 'rws',
            'ngo'         => 'ngo',
        ...
        if (in_array($consumerType, array_keys($jobTypes))) $data['job_type'] = $jobTypes[$consumerType];
        ...
        $data['job_for'] = $this->buildJobFor($job, $data);
...
//        $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
        return [
            ...
            'type'                   => $data['immediate'] ? 'immediate' : 'regular'
        ];
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $this->sendBookingInformationMail($tr, $job, $session_time, $completeddate, $post_data['user_id']);
    }
    ...
    public function isNeedToDelayPush($user_id)
    {
        ...
        if (TeHelper::getUsermeta($user_id, 'not_get_nighttime') == 'yes') return true;
        return false;
    }
    ...
    {
        if (TeHelper::getUsermeta($user_id, 'not_get_notification') == 'yes') return false;
        return true;
    }
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        if (!empty($job->certified)) {
            if ($job->certified == 'yes' || $job->certified == 'both') {
                ...
                $translator_level[] = 'Certified with specialisation in health care';
                //both condition here won't as top condition is also there
            } else if ($job->certified == 'normal' || $job->certified == 'both') {
                $translator_level[] = 'Layman';
                $translator_level[] = 'Read Translation courses';
            }
            // this elseif won't work becuase on top there is condition which checking $job->certified is not empty  so i added else down there
...
//        }
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        $data = $this->buildJobToData($job);            // save job's information to data for sending Push
     ...
     * @return array
     */
    protected function buildJobFor($job, $data)
    {
        $data['job_for'] = [];
        if ($job->gender != null) {
            if ($job->gender == 'male') {
     ...
     * @return array
     */
    protected function buildJobToData($job)
    {
        $data = [];
        $data['job_id'] = $job->id;
     ...
     * @return void
     */
    protected function sendBookingInformationMail($tr, $job, $session_time, $completeddate, $user_id)
    {
        $user = $tr->user()->first();
        $email = $user->email;
        $name = $user->name;
